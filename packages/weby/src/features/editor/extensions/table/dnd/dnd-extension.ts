...
import { cellInfoFromResolvedCell, getDndRelatedDOMs, getHoveringCell } from "./utils";
...
class TableHandlePluginSpec implements PluginSpec<TableHandleState> {
  ...
  startDragFromHandle = (orientation: "col" | "row", clientX: number, clientY: number): boolean => {
    ...
    const draggingIndex =
      (orientation === "col" ? this._hoveringCell.colIndex : this._hoveringCell.rowIndex) ?? 0;
  ...
  commitDrop = () => {
    ...
    if (direction === "col") {
      if (moveColumn({ originIndex: from, pos, select: true, targetIndex: to, tr })) {
        this.editor.view.dispatch(tr);
      }
      return;
    }
    if (moveRow({ originIndex: from, pos, select: true, targetIndex: to, tr })) {
      this.editor.view.dispatch(tr);
    }
  };
...
export function getTableHandlePluginSpec(editor: Editor): TableHandlePluginSpec | null {
