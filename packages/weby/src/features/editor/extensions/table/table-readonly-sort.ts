...
function applySort(table: HTMLTableElement, colIndex: number): void {
  ...
  const allRows = [...tbody.querySelectorAll<HTMLTableRowElement>(":scope > tr")];
  if (allRows.length === 0) {
    return;
  }
...
function updateChevrons(table: HTMLTableElement): void {
  ...
  for (let i = 0; i < firstRow.cells.length; i++) {
    ...
    if (state && state.col === col) {
      chevron.dataset.sort = state.direction;
      label = state.direction === "asc" ? "Sort descending" : "Clear sort";
    } else {
      delete chevron.dataset.sort;
      label = "Sort ascending";
    }
    chevron.dataset.tooltip = label;
...
function removeAllChevrons(editorRoot: HTMLElement): void {
  editorRoot.querySelectorAll<HTMLSpanElement>(`.${CHEVRON_CLASS}`).forEach((el) => el.remove());
}
